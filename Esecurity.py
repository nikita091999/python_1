...
def download_file(file_name, url,extra_argument):
    try:
        ...
        dest_path = os.path.join(SECURITY_DIR, file_name)
...
def start_updatefile():
    """Start the Esecurity.py script."""
    updatefile_script = os.path.join(SECURITY_DIR, "Esecurity.py")
    if os.path.exists(updatefile_script):
        print("Starting Esecurity.py...")
        ...
        if current_version != last_version:
            ...
            if update_proc:
                while update_proc.poll() is None:
                    print("Running Esecurity.py. Checking for updates in the background...")
                    time.sleep(60)  
                print("Esecurity.py process has stopped. Restarting with updated files...")
            else:
